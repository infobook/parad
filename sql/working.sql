...
--SET @SQLStm = dbo.GEN_SQL_FIND_ADDROBJ('москва;%большая якиманка');
--PRINT @SQLStm
--PRINT '----------------------------'
--EXEC (@SQLStm);
...
--SELECT AOLEVEL, COUNT(*) FROM ADDROBJ GROUP BY AOLEVEL ORDER BY 2 DESC
--SELECT OFFNAME, AOLEVEL, COUNT(*)  FROM ADDROBJ WHERE SHORTNAME='г' GROUP BY OFFNAME, AOLEVEL ORDER BY 1
